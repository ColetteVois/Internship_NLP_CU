...
for(j in 1:n.tokenizer.sentence){
 ...
 temp <- paste(my_path,sprintf("/preprocessing/tokenizer_sentence/tokenizer_sentence_%d.R", j), sep="")
...
for(j in 1:n.tokenizer.word){
  ...
  temp <- paste(my_path,sprintf("/preprocessing/tokenizer_word/tokenizer_word_%d.R", j), sep="")
  myfiles <- lapply(temp, read.csv, sep="\n", fill = TRUE , header = FALSE,  col.names = "text", stringsAsFactors = FALSE)#fill = TRUE,header = FALSE,stringsAsFactors = FALSE)#, col.names = rep("text",length(listfiles)+1))
...
for(j in 1:n.normalization){
  ...
  temp <- paste(my_path,sprintf("/preprocessing/normalization/normalization_%d.R", j), sep="")
  myfiles <- lapply(temp, read.csv, sep="\n", fill = TRUE , header = FALSE,  col.names = "text", stringsAsFactors = FALSE)#fill = TRUE,header = FALSE,stringsAsFactors = FALSE)#, col.names = rep("text",length(listfiles)+1))
