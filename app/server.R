...
server <- function(input, output, session){
  ...
  output$box_1 <- renderPlotly({
    plot_ly(test_d,x = rep(0, length(test)), y=~test, type = "scatter", source = "box1", mode='markers')%>%add_trace(test_d, y=~test, type = "box", hoverinfo = 'text', text =~paste("Maximum:", fivenum(test_d)[5], "Q3:", fivenum(test_d)[4]), marker = list(outliercolor = "red"))
  })
  output$box_2 <- renderPlotly({
    plot_ly(test_d,x = rep(0, length(test)), y=~test, type = "scatter", source = "box2", mode='markers')%>%add_trace(test_d, y=~test, type = "box",  marker = list(outliercolor = "red"))
  })
  output$box_3 <- renderPlotly({
    plot_ly(test_d,x = rep(0, length(test)), y=~test, type = "scatter", source = "box3", mode='markers')%>%add_trace(test_d, y=~test, type = "box",  marker = list(outliercolor = "red"))
  })
