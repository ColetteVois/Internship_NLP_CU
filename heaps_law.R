...
heaps.law <- function(original_books_select, choose_tokenizer_sentence, choose_tokenizer_word) {

  #choose_tokenizer_sentence <- 1
  #choose_tokenizer_word <- 1
  
  book.length <- nrow(original_books_select)
  ...
  for(i in 1:(10^accura)) {
    ...
    original_books2 <- as_tibble(original_books_select[1:extrait.length,1:2])
...
heaps.law(original_books_select, 1, 1)
